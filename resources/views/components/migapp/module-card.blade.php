...
<div class="card-hover animate-fadeIn" style="animation-delay: {{ $animationDelay }}s;">
    <a href="{{ $routeUrl }}" 
       class="block bg-white/95 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-white/20 hover:border-{{ $color }}-400 transition-all group {{ !$isRouteAvailable ? 'cursor-not-allowed opacity-75' : '' }}"
       @if(!$isRouteAvailable) onclick="return false;" @endif>
        ...
        <div class="text-center">
            ...
            <!-- Descripción -->
            <p class="text-sm text-gray-600 mb-4 group-hover:text-gray-700 transition-colors">
                {{ $description }}
            </p>
            
            <!-- Botón de acceso -->
            <div class="inline-flex items-center text-{{ $color }}-600 font-medium text-sm group-hover:text-{{ $color }}-700 transition-colors">
